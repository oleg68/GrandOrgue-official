 ...
 * writes a new constant into a target GOAddConstProcessor, to demonstrate a
 * mapper's real role of pushing a value into a processor's parameters.
 */
class GOCountingMapper : public GOSoundProcessingPrmMapper {
private:
  // Number of EnsureParametersUpToDate() calls seen so far.
  unsigned m_NCalls = 0;
  // Optional processor to push a new constant into; null if this instance
  // only counts calls.
  GOAddConstProcessor *p_target;
  // New constant to write into *p_target, if p_target is set.
  const float m_newValue;

public:
  explicit GOCountingMapper(
    GOAddConstProcessor *pTarget = nullptr, float newValue = 0);
