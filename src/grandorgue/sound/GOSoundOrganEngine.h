...
private:
  ...
  //   — referenced by: m_RecorderTask [B4], reverb setup [B5]
  ...
  // [B4] recorder: set up sample rate and outputs on m_RecorderTask
  //   — uses mp_DownmixTask [B3] or m_OutputStates [B2]
...
public:
   ...
   * After return the engine is ready to receive ProcessAudioCallback() calls.
