...
void GOToolbarWindow::OnBeforeSoundClose() { EnsureOrganStopped(); }

bool GOToolbarWindow::EnsureSoundMidiOpen() {
...
void GOToolbarWindow::EnsureSoundMidiClosed() {
  r_SoundSystem.AssureSoundIsClosed();
}

void GOToolbarWindow::EnsureOrganStartedIfReady() {
  if (
...
void GOToolbarWindow::EnsureOrganStopped() {
...
void GOToolbarWindow::OnMeters(wxCommandEvent &event) {
  if (m_isMeterReady) {
    const std::vector<double> vals
      = (p_OrganController && p_OrganController->IsStarted())
